...
static int draw_background(void)
{
    ...
    // workers
    draw_title(66, 1);
    if (city_labor_unemployment_percentage() > 4) {
    ...
    } else if (city_labor_unemployment_percentage() > 0) {
        width = lang_text_draw(61, 12, X_OFFSET, 66, FONT_NORMAL_GREEN);
        width += text_draw_percentage(city_labor_unemployment_percentage(), X_OFFSET + width, 66, FONT_NORMAL_GREEN);
        text_draw_number(city_labor_workers_unemployed() - city_labor_workers_needed(), '(', ")",
            X_OFFSET + width, 66, FONT_NORMAL_GREEN);
